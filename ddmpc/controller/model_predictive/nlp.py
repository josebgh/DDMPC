...
class NLP:
    ...
    def __init__(
        ...
        self._par_vars:         list[NLPVariable] = list()
        self._opt_vars:         list[NLPVariable] = list()
        self._opt_vars_ss:      list[NLPVariable] = list()
        self._var_map:          dict[tuple[Source, int], NLPVariable] = dict()
        self._var_map_ss:       dict[tuple[Source, int], NLPVariable] = dict()
        self._inp_map:          dict[Predictor, dict[int, list[NLPVariable]]] = dict()
    ...
    def _create_ss_variables(self):
        
        # par_vars are the same in ss and non-ss, but the name of the source is changed
        self._par_vars_ss = copy.deepcopy(self._opt_vars)
        sources_modified = []
        for opt_var_ss in self._par_vars_ss:
            if opt_var_ss.feature.source not in sources_modified:
                sources_modified.append(opt_var_ss.feature.source)
                opt_var_ss.feature.source.name=opt_var_ss.feature.source.name+"_ss"

        # opt_vars
        self._opt_vars_ss = copy.deepcopy(self._opt_vars)
        sources_modified = []
        for opt_var_ss in self._opt_vars_ss:
            if opt_var_ss.feature.source not in sources_modified:
                sources_modified.append(opt_var_ss.feature.source)
                parenthesis = ""
                while opt_var_ss.feature.source.name[-1]==")":
                    parenthesis = parenthesis + opt_var_ss.feature.source.name[-1]
                    opt_var_ss.feature.source.name=opt_var_ss.feature.source.name[0:-1]
                opt_var_ss.feature.source.name=opt_var_ss.feature.source.name+"_ss"+parenthesis

        # var_map
        self._var_map_ss = copy.deepcopy(self._var_map)
        sources_modified = []
        for (source,k) in self._var_map.keys():
            if source not in sources_modified:
                sources_modified.append(source)
                source_ss = copy.deepcopy(source)
                parenthesis = ""
                while source_ss.name[-1]==")":
                    parenthesis = parenthesis + source_ss.name[-1]
                    source_ss.name=source_ss.name[0:-1]
                source_ss.name=source_ss.name+"_ss"+parenthesis
                var_ss = self._var_map_ss[source,k]
                print(var_ss.feature.source.name)
                parenthesis = ""
                while var_ss.feature.source.name[-1]==")":
                    parenthesis = parenthesis + var_ss.feature.source.name[-1]
                    var_ss.feature.source.name=var_ss.feature.source.name[0:-1]
                var_ss.feature.source.name=var_ss.feature.source.name+"_ss"+parenthesis
                self._var_map_ss[source,k] = var_ss
                self._var_map_ss[source_ss,k] = self._var_map_ss.pop(source,k)


    def _connect_constructed_ss(self):
        ...
        for c in self.model.connecting:
            ...
            for k in range(0, self.N + 1):
                ...
                source_ss = copy.deepcopy(c.source)
                parenthesis = ""
                while source_ss.name[-1]==")":
                    parenthesis = parenthesis + source_ss.name[-1]
                    source_ss.name=source_ss.name[0:-1]
                self._constraints.append(
                    NLPConstraint(expression=source_ss.constraint(k))
                )
