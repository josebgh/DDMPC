...
mu = sdpvar(ny*ones(1,N),ones(1,N),'full');
...
for k=1:N
    ...
    for i=1:length(yss{k})
        if eps_vars_Eco(i)==1
            cost = cost + [ eps_Eco{k}(i)' * mu{k}(i) * eps_weights_Eco(i,i) * eps_Eco{k}(i) ];
    ...
    % Quadratic Cost
    cost = cost + [ ( yss{k} - y_ref{k} )' * diag(mu{k}) * S_q * ( yss{k} - y_ref{k} ) ];

    % Absolute linear Cost
    for i=1:length(yss{k})
        if eps_vars_AbsLin(i)==1
            cost = cost + [ mu{k}(i) * eps_weights_AbsLin(i) * eps_AbsLin_1{k}(i) ];
            cost = cost + [ mu{k}(i) * eps_weights_AbsLin(i) * eps_AbsLin_2{k}(i) ];
